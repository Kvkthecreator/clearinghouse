...
export function ChatFirstLayout({
  ...
  const handleTabChange = useCallback((tab: DetailTab) => {
    setDetailState((prev) => ({
      ...prev,
      activeTab: tab,
      itemId: undefined, // Clear item focus when switching tabs
    }));
  }, []);
...
// Provider wrapper
export function ChatFirstLayoutProvider({
  children,
  ...layoutProps
}: ChatFirstLayoutProps) {
  const [layoutActions, setLayoutActions] = useState<ChatFirstLayoutContextValue>({
    openDetailPanel: () => {},
    closeDetailPanel: () => {},
  });

  return (
    <ChatFirstLayoutContext.Provider value={layoutActions}>
      <ChatFirstLayout {...layoutProps}>
        {children}
      </ChatFirstLayout>
